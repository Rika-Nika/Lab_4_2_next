...
inline int HashFunctionHornerExt(const std::wstring& s, int table_size, const int key)
{
	int hash_result = 0;
...
template<class T>
inline int HashFunctionExt(const int index, int table_size, const int key)
{
...
template<class T>
struct HashFunctionExt1
{
	int operator()(const T& p, int table_size) const
	{
		std::wstring s = p.GetIndex();
		return HashFunctionHornerExt(s, table_size, table_size - 1); // ключи должны быть взаимопросты, используем числа <размер таблицы> плюс и минус один.
	}
};
template<class T>
struct HashFunctionExt2
{
	int operator()(const T& p, int table_size) const
	{
		std::wstring s = p.GetIndex();
		return HashFunctionHornerExt(s, table_size, table_size + 1);
	}
};


template <class T, class THash1 = HashFunctionExt1<T>, class THash2 = HashFunctionExt2<T>>
class HashTableExt :public IDictionary<T, T>
{
...
int HashTableExt<T, THash1, THash2>::GetSize() const
{
	return size;
}

template <class T, class THash1, class THash2>
HashTableExt<T, THash1, THash2>::HashTableExt()
{
...
template <class T, class THash1, class THash2>
void HashTableExt<T, THash1, THash2>::Resize()
{
...
template <class T, class THash1, class THash2>
void HashTableExt<T, THash1, THash2>::Rehash()
{
	size_all_non_nullptr = 0;
	size = 0;
	Node** arr2 = new Node * [capacity];
...
template <class T, class THash1, class THash2>
T HashTableExt<T, THash1, THash2>::Find(const T& value, const THash1& hash1, const THash2& hash2)
{
...
template <class T, class THash1, class THash2>
T HashTableExt<T, THash1, THash2>::FindLast(int count, const THash1& hash1, const THash2& hash2)
{
	T res = T();
	int i = 0;
...
template <class T, class THash1, class THash2>
bool HashTableExt<T, THash1, THash2>::Remove(const T& value, const THash1& hash1, const THash2& hash2)
{
...
template <class T, class THash1, class THash2>
bool HashTableExt<T, THash1, THash2>::Add(const T& value, const THash1& hash1, const THash2& hash2)
{
...
template <class T, class THash1, class THash2>
T HashTableExt<T, THash1, THash2>::Get(T key) {
	return Find(key);
}

template <class T, class THash1, class THash2>
bool HashTableExt<T, THash1, THash2>::ContainsKey(T key)
{
	return !Find(key).IsNull();
}

template <class T, class THash1, class THash2>
void HashTableExt<T, THash1, THash2>::AddItem(const T key, const T elem)
{
	HashTableExt::Add(key);
}

template <class T, class THash1, class THash2>
void HashTableExt<T, THash1, THash2>::RemoveItem(T key)
{
	HashTableExt::Remove(key);
}

template <class T, class THash1, class THash2>
int HashTableExt<T, THash1, THash2>::GetCapacity()
{
	return capacity;
}
